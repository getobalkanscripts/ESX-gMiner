        ...
        if not Config.MiningLocation[mining]["isMined"] and not Config.MiningLocation[mining]["isOccupied"] then
          ESX.TriggerServerCallback('esx-mining:pickaxe', function(PickAxe)
            ...
            elseif not PickAxe then
              exports['mythic_notify']:DoHudText('error', 'Nemas kramp koji ti je potreban da bi zapoceo kopanje')
...
RegisterNetEvent('esx-mining:washingrocks', function()
  ESX.TriggerServerCallback('esx-mining:washpan', function(washingpancheck)
    if washingpancheck then
        ...
        if stonesbruf then
          ...
          TaskStartScenarioInPlace(playerPed, 'WORLD_HUMAN_BUM_WASH', 0, false)
            TriggerEvent("mythic_progbar:client:progress", {
              ...
              ClearPedTasks(PlayerPedId())
              exports['mythic_notify']:DoHudText('inform', 'Prekinuo si proces pranja kamenja')
             -- ESX.ShowNotification(Config.Text['cancel'])
            end
          end)
        else
          exports['mythic_notify']:DoHudText('error', 'Nemas ni jedan kamen za pranje')
      ...
      exports['mythic_notify']:DoHudText('error', 'Treba ti cetka za pranje da bi prao kamenje!')
    end
  end)
end)
    ...
    if IronCheck then
      ...
      TriggerEvent("mythic_progbar:client:progress", {
       ...
       --   TriggerEvent('animations:client:EmoteCommandStart', {"c"})
          ...
          ClearPedTasks(PlayerPedId())
          exports['mythic_notify']:DoHudText('error', 'Prekinuo si proces topljenja metala!')
          --ESX.ShowNotification(Config.Text['cancel'])
        end
      end)
    else
      exports['mythic_notify']:DoHudText('error', 'Nema dovoljno Iron Ores da bi ih mogao istopiti!')
    ...
    if CopperCheck then
      ...
      --TriggerEvent('animations:client:EmoteCommandStart', {"Warmth"})
        TriggerEvent("mythic_progbar:client:progress", {
          ...
          ClearPedTasks(PlayerPedId())
          exports['mythic_notify']:DoHudText('error', 'Prekinuo si topljenje bakra!')
         -- ESX.ShowNotification(Config.Text['cancel'])
        end
      end)
    else
      exports['mythic_notify']:DoHudText('error', 'Nemas dovoljno bakra za topljenje!')
    ...
    if GoldCheck then
     ...
     -- TriggerEvent('animations:client:EmoteCommandStart', {"Warmth"})
        ...
        --  TriggerEvent('animations:client:EmoteCommandStart', {"c"})
          ...
          ClearPedTasks(PlayerPedId())
          exports['mythic_notify']:DoHudText('error', 'Prekinuo si topljenje Zlata!')
         -- ESX.ShowNotification(Config.Text['cancel'])
        end
      end)
    else
      exports['mythic_notify']:DoHudText('error', 'Nemas dovoljno zlatnih orova za topljenje!')
